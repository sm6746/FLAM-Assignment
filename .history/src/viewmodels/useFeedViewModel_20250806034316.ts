...
export function useFeedViewModel(): FeedViewModel {
  ...
  const loadFeedPage = useCallback(async (page: number, append: boolean = false) => {
    try {
      ...
      // Calculate heights for new items
      ...
      updateState(prev => ({
        ...
        hasMore: newItems.length === 10, // Assume no more if less than page size
        error: null,
